...
router.get('/maps-files-tree', cors(CORSoptions), (req: any, res: any) => { // GET Files(maps) tree
...
router.get('/map-file', cors(CORSoptions), (req: any, res: any) => { // GET Files(maps) tree
  if (!req.headers.authorization) return res.sendStatus(UNAUTHORIZED);
...
router.post('/upload-map', cors(CORSoptions), upmap.fields([{ name: 'file', maxCount: 10 }]), (req: any, res: any) => { // POST Rewrite changed config(any) file
...
router.post('/upload-cfg', cors(CORSoptions), upcfg.fields([{ name: 'file', maxCount: 10 }]), (req: any, res: any) => { // POST Rewrite changed config(any) file
  if (!req.headers.authorization)  { res.sendStatus(UNAUTHORIZED); return ; }
  Logger.log('default', 'POST │', req.connection.remoteAddress, '\x1b[94m', req.user.user,`\x1b[91mrole: \x1b[93m${req.user.group_id}`, '\x1b[0m' ,'-> UPLOAD_FILE', /**req.body.file.path,**/ '[', req.originalUrl, ']');
  res.sendStatus(OK);
});
