...
router.get('/config-files-tree', cors(CORSoptions), (req: any, res: any) => { // GET Files(configs) and directories tree
    ...
    router.get('/config-file', cors(CORSoptions), (req: any, res: any) => { // GET single config file
      if (!req.headers.authorization) return res.sendStatus(UNAUTHORIZED);
    ...
    router.post('/save-config', cors(CORSoptions), json(), (req: any, res: any) => { // POST Write map file
