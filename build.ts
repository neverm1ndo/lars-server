...
const CONSOLE_MESSAGE = {
    ...
    POST_BUILD: {
        ...
        JISON_GEN: (curr?: number, all?: number) => `Generating parsers ${curr ?? 0}/${all ?? 0} :: Jison Genetator`,
    },
...
(async function build(config) {
    try {
        ...
        for (let grammar of grammars) {
            console.info(CONSOLE_MESSAGE.get('POST_BUILD.JISON_GEN', current++, grammars.length));
